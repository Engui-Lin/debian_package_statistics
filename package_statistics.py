...
def print_top_10_packages(architecture: str, package_stats: dict):
    ...
    top_10_packages = sorted(package_stats.items(), key=lambda item: item[1], reverse=True)[:10]
...
def main():
    parser = argparse.ArgumentParser(description='Get statistics of debian packages with most files.')
    parser.add_argument(
        'architecture', 
        nargs='?',
        type=str,
